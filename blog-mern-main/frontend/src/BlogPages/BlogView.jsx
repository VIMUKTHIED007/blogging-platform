...
function BlogView() {
    ...
    const fetchDetailsData = () => {
        ...
        let url;
        if (isAuthenticated) {
            url = `http://localhost:3000/blog/${id}/details`;
        } else {
            url = `http://localhost:3000/blog/${id}/details/public`;
        }
    ...
    const fetchData = () => {
        axios.get(`http://localhost:3000/blog/blogs/${id}`, {}, { withCredentials: true })
            .then((res) => {
                setLoading(false)
                setData(res.data)
    ...
    const viewCountAPI = () => {
        axios.post(`http://localhost:3000/blog/${id}/view-count`, {}, { withCredentials: true })
    }
    ...
    const handleLike = () => {
        axios.post(`http://localhost:3000/blog/${id}/like`, {}, { withCredentials: true })
            .then((res) => {
                fetchDetailsData();
    ...
    const handleDislike = () => {
        axios.post(`http://localhost:3000/blog/${id}/dislike`, {}, { withCredentials: true })
            .then((res) => {
